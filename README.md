...
## Агрегация
Выходные данные объединяются в `general.jsonl` и приводятся к формату `general.csv` таблицы.

## Визуализация
Данные из `geberal.csv` используются для графического отображения необходимых зависимостей.

## Результат
Результатом является отчёт в формате RADME.md, где используются построенные графические зависимости и полученные замеры.
