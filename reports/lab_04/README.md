...
Каждый CUDA-поток отвечает за вычисление одного элемента результирующей матрицы.
...
* `O(n * m * p)` — вычислительная сложность матричного умножения без учёта параллельного выполнения на GPU;
* `O(n * m + m * p + n * p)` — общий объём памяти, необходимый для хранения входных матриц `A`, `B` и результирующей матрицы `C`;
* при идеализированном распределении вычислений между `T` CUDA-потоками вычислительную часть можно оценить как `O(n * m * p / T)`. Это теоретическая оценка: реальное время выполнения зависит от количества одновременно выполняемых потоков, размера CUDA-блока, количества блоков, архитектуры GPU и других факторов.

Для квадратных матриц одинаковой размерности `N` последовательная вычислительная сложность составляет `O(N³)`, а при идеализированном параллельном выполнении — `O(N³ / T)`, где `T` характеризует степень доступного параллелизма.

При этом полный цикл выполнения CUDA-реализации включает не только вычисление матриц, но и передачу данных между памятью CPU и GPU. Поэтому фактическое время выполнения может отличаться от идеализированной оценки.
...
Из 56 запусков все 56 признаны корректными (`valid = True`). Максимальная абсолютная ошибка по всем запускам составила `5,28·10⁻¹¹`, что значительно меньше заданного допуска `atol = 1e-6`.
...
С увеличением размера матрицы время выполнения в целом возрастает, что соответствует кубической вычислительной сложности классического алгоритма матричного умножения.

При небольших размерах матриц разница между конфигурациями блока сравнительно невелика. При увеличении `N` различия между конфигурациями становятся заметнее.
...
При отдельных размерах наблюдаются локальные отклонения производительности, например при `N = 1200` для блока `4 × 4` и при `N = 1000` для блока `32 × 32`.
...
При увеличении размера матрицы производительность в целом возрастает. Для небольших размеров матриц производительность ниже, чем для больших задач.
...
Для блока `4 × 4` производительность на больших размерах матриц заметно ниже, чем для остальных конфигураций. При `N = 3000` значение составляет `93.82 GFLOP/с`, тогда как для блоков `8 × 8`, `16 × 16` и `32 × 32` оно находится около `209 GFLOP/с`.

При этом увеличение размера блока с `16 × 16` до `32 × 32` не приводит к заметному изменению производительности на больших размерах задачи.
...
* Увеличение размера блока с `16 × 16` до `32 × 32` на исследованных больших размерах не привело к заметному изменению производительности.
